                ...
                sql.all("select staffId from stafftb where designation=?",
                    ...
                    function (err, res) {
                        ...
                        if (err) {
                            ...
                            const staffId = res[Math.floor(Math.random() * res.length)].staffId;
...
exports.getTodayAppointments = (result) => {
    ...
    patienttb.firstName, patienttb.lastName, appointmenttb.appointmentId,\
    appointmenttb.staffId from \
...
exports.makeReport = (diagnosis,
    appointmentId, treatment, result) => {
    sql.run("update appointmenttb set diagnosis=?,\
     status=? where appointmentId=?",
        [diagnosis, "seen", appointmentId],
        function (err) {
            if (err) {
                result(err);
            }
            else if (this.changes > 0 && treatment.length) {
...
exports.getMedicalReports = (patientId, result) => {
    ...
    sql.all("select appointmentId,appointmentDate,diagnosis, staffId  from appointmenttb\
