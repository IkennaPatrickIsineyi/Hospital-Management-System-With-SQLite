...
exports.createAppointmenttb = (result) => {
    ...
    primary key(appointmentId), index(patientId),index(staffId),\
...
exports.createPatientAssignmenttb = (result) => {
    sql.query("create table if not exists patientAssignmenttb(id int\
    auto_increment, staffId varchar(12), qty int,\
    check(qty>=0),primary key(id),index (staffId),\
...
exports.createMapConsultTrigger = (result) => {
    sql.query("create trigger if not exists map_consult_app\
    ...
    values ( ?,?,?)", [patientId, appointmentDate, "pending"],
        (err, res1) => {
            if (err) {
                ...
                sql.query("select staffId from stafftb where designation=?",
                    ["consultant"],
                    (err, res) => {
                        if (err) {
                            result(err, res);
                            ...
                            sql.query("update appointmenttb set staffId=? where appointmentId=?",
                                [staffId, res1.insertId], result);
                        }
                    });
            ...
            if (err) {
                ...
                if (!res1.length) {
                    ...
                    for (let item of res1) {
                        sql.query("select stocktb.name, reqItemtb.qty, reqItemtb.note\
