                    ...
                    function (err) {
                        if (err) {
                            ...
                            homePageSetup(req.body.staffId, req, res, () => {
                                res.cookie("staffId", req.body.staffId, { sameSite: 'none', secure: true });
    ...
    checkLoginStatus(req.cookies.staffId, req.cookies.sessionId,
        (isLoggedIn, error) => {
            ...
            if (error) {
                ...
                saveFile(req.files.image, req.body.firstname, 'public/staff', (fileSaved, myFileName) => {
                    if (fileSaved) {
                        console.log("checked");
                        ...
                        db.createAccount(staffId, password,
                            ...
                            myFileName, req.body.birthday, function (err, result) {
                                if (err) {
                                    ...
                                    res.send({
                                        respCode: 1,
                                        data: { 'staffId': staffId, 'pic': myFileName, 'fullName': firstName + ' ' + lastName }
                                    });
                                }//else if(this.changes)
                                else {
        ...
        (isLoggedIn, error) => {
            if (error) {
                ...
                saveFile(req.files.image, req.body.firstName,
                    'public/patient', (fileSaved, myFileName) => {
                        if (fileSaved) {
...
//files
exports.staffFiles = (req, res) => {
    const filePath = path.join(__dirname, '..', 'public', 'staff', req.query.filename);
    console.log(filePath);
    res.sendFile(filePath);
};

//files
exports.patientFiles = (req, res) => {
    const filePath = path.join(__dirname, '..', 'public', 'patient', req.query.filename);
    console.log(filePath);
    res.sendFile(filePath);
};
//uploadFile
